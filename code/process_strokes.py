...
Utility functions to process strokes / bitmaps created from strokes.  

The bitmaps will be stored in ../pickle_files/features/clean_symbols.p
'''
...
# where are we saving our pickle file to? include pixel dimension in file name
OUTPUT_LOCATION = "../pickle_files/features/clean_symbols_" + str(BOUND_DIM) + ".p"
...
# special symbols which will be recognized as a concatenation of sub symbols
MULT_COMP_SYMBOLS = {
                '=':2,
                'i':2,
                '\tan':4,
...
# symbols that are look exactly and which are distinguishible only by context
DICT_MAP = {
            '\\times':'x',
            ',':'/'
            }
...
def smooth_and_norm(img):
    """
    Apply a Gaussian filter to the image then divide by its two norm. 
    """
    img = img * 1.0
...
def normal_treatment(symbol,trace_list,bitmap_list,symbol_list,plot_me=False):
    ...
    # smooth and normalize bitmap
    new_cur_data = smooth_and_norm(new_cur_data)
...
def xy_to_bitmap(data_arr):
    ...
    First: scale xy coordinates so that everything is between 0 and 28. 
    ...
    #min_x = max(data_arr[:,0])
    #min_y = max(data_arr[:,0])
    ...
    diff_x = 0.5*(BOUND_DIM - (max(data_arr[:,0]) - min(data_arr[:,0])))
    diff_y = 0.5*(BOUND_DIM - (max(data_arr[:,1]) - min(data_arr[:,1])))
    
    data_arr[:,0] = data_arr[:,0] + diff_x
    data_arr[:,1] = data_arr[:,1] + diff_y
    ...
    # TODO can this be done using an anti aliasing algorithm? 
    for elem in data_arr: 
        vis[-1 + FULL_DIM - int(elem[1]),-1 + int(elem[0])] = 1
...
def lin_interp_stroke(data_arr):
    ...
    x_abs_range = abs( data_arr[0,:].max() - data_arr[0,:].min()) 
    y_abs_range = abs( data_arr[1,:].max() - data_arr[1,:].min()) 
    
    # we select interpolation based on how big the symbol is 
    # this is because some of the data is on a different range
    if x_abs_range < 200 and y_abs_range < 200:
        interp_res = 0.001
    else:
    ...
    for ind in range(data_len-1):  
        ...
        if x_diff > y_diff: 
            ...
            x_new = np.arange(0.0001 + min(data_arr[ind:ind+2,0]),-0.0001 + max(data_arr[ind:ind+2,0]),interp_res)
            y_new = g(x_new)
        ...
        # if the two points between which we are interpolating is tall
            ...
            y_new = np.arange(0.0001 + min(data_arr[ind:ind+2,1]), -0.0001 + max(data_arr[ind:ind+2,1]),interp_res)
            x_new = g(y_new)
...
def loop_over_data():
    ...
    print "Extracting symbol images..."
    xml_file_list = glob.glob(os.path.join(REL_PATH,"*.inkml"))    
    symbol_list = [] 
    bitmap_list = []
    ct = 0
    data_len = len(xml_file_list)
    ...
    for cur_file in xml_file_list:
        ...
        for traceGroup in traceGroup_parent: 
            ...
            if symbol in SYM_CASE_SAME:
                symbol = symbol.lower()
                
            if symbol in DICT_MAP.keys():
                symbol = DICT_MAP[symbol]
    ...
    with open(OUTPUT_LOCATION,'wb') as f:
        ...
        print "Successfully saved pickle file to: " + str(OUTPUT_LOCATION)
